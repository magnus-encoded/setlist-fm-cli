...
| **The step and the turn** | How a photograph moves as the walk goes past it. Two halves, in order. **The rank**: everything not yet reached stands square to the walker, and because they share the flank the nearer ones project further out — a picket of overlapping edges receding to the vanishing point. **The step**: as the walk reaches it, one photograph slides in off the wall toward the spine, standing furthest into the aisle exactly where a tap would take it — half the flank in, which is as far as it can go before the two flanks meet. One gap wide, so exactly one photograph is ever out of line. Out of line means yours, and it is the only one that can be seen whole. Measured *after* the projection: a card on its way in is also swelling, and an inward step in world units can be cancelled by the swelling exactly, leaving it tracking a straight line up the screen. **The turn**: past the **Focal plane** it swings round to parallel with the walk, stepping back to the wall and fading as it goes, and is **gone the instant it reaches parallel** — turning further is a card showing its back edge, which reads as swinging through the walker. The turn is no wider than the tightest packing, so only ever one photograph is mid-turn and the one after it has already left. Photographs alone leave here; the floor and the spine keep the shared window, because the road does run under you. | tilt, rotation, flip, slide |
